...
No host installation of tarawasm is needed. Every build runs the pinned image with the repository mounted at `/work`.
...
`wit-deps/` contains pinned WIT package components plus `SHA256SUMS`. Every successful `make build` strips and validates the final component before leaving it in `dist/adder.wasm`. The HTTP callback interface `docs:adder/lua-callback@0.1.3` is a contract provided by the `tarantool-wasm-rs` library; it is not part of `tarantool-wit`.
...
- tarawasm schema or CLI mismatch: pull `mandeser0/tarawasm:v0.3.0`; legacy `--wasm-file` configs are not supported here.
