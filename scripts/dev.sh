...
tarawasm)
    ...
    image=${TARAWASM_IMAGE:-mandeser0/tarawasm:v0.3.0}
    ...
    while IFS= read -r config; do
        ...
        if [ "$#" -eq 1 ] && [ "$1" = all ]; then
            docker run --rm -v "$repo_dir:/work" -w "/work/$relative" \
                "$image" strip dist/adder.wasm
            mv "$component_dir/dist/adder.strip.wasm" "$component_dir/dist/adder.wasm"
            wasm-tools validate "$component_dir/dist/adder.wasm"
        fi
    done
    ;;
tarawasm-one)
    ...
    if [ "$#" -eq 1 ] && [ "$1" = all ]; then
        docker run --rm -v "$repo_dir:/work" -w "/work/$relative" \
            "$image" strip dist/adder.wasm
        mv "$sample_dir/dist/adder.strip.wasm" "$sample_dir/dist/adder.wasm"
        wasm-tools validate "$sample_dir/dist/adder.wasm"
    fi
    ;;
deps)
