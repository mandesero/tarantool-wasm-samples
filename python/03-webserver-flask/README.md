...
Flask and all transitive packages are pinned in `requirements.txt`. The request and response are Lua tables encoded by the runtime as MessagePack; no Lua function or state crosses into guest memory. `docs:adder/lua-callback@0.1.3` is currently a runtime-local experimental interface, not a stable published Tarantool WIT package. Omit `SAMPLE_PORT` to use 8080; `make smoke` chooses a free port and checks that it is released.
