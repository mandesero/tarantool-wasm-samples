...
local host = os.getenv('POSTGRES_HOST') or '127.0.0.1'
...
local tls_server_name = assert(os.getenv('POSTGRES_TLS_SERVER_NAME'), 'POSTGRES_TLS_SERVER_NAME is required')

runner.run(dir .. 'dist/adder.wasm', runner.default_options({
    args = {'postgres', host, tostring(port), user, password, database, message,
            tls_server_name},
