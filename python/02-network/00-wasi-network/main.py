...
from wit_world.imports import instance_network, network, poll, say, tcp, tcp_create_socket

import sys


def run_client(port: int) -> None:
    say.say_info("PY | ===== Raw WASI TCP start =====", None)
    with instance_network.instance_network() as net:
        with tcp_create_socket.create_tcp_socket(network.IpAddressFamily.IPV4) as sock:
            ...
            with input_stream, output_stream:
                ...
                say.say_info(f"PY | Echo: {echoed.decode().strip()}", None)
            sock.shutdown(tcp.ShutdownType.BOTH)
    say.say_info("PY | ===== Raw WASI TCP done =====", None)
