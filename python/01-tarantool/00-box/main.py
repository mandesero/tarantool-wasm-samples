...
from wit_world.imports.types import Space, Index, IteratorType
from wit_world.imports import index as index_api
import json
...
def test_box():
    ...
    for row in test_data[:2]:
        ...
        say.say_info(f"PY | Inserted: {row}, tuple_ptr={hex(inserted.ptr)}, decoded={decode(data=box_tuple.to_buf(inserted))}", None)
    ...
    say.say_info(f"PY | Updated id=1 (age=26), tuple_ptr={hex(updated.ptr)}, decoded={decode(data=box_tuple.to_buf(updated))}", None)

    # Replace: completely replace tuple for id=2
    replaced = box.replace(space, encode([2, "Bob Jr.", 31]))
    say.say_info(f"PY | Replaced id=2 -> ['Bob Jr.', 31], tuple_ptr={hex(replaced.ptr)}, decoded={decode(data=box_tuple.to_buf(replaced))}", None)
    ...
    say.say_info(f"PY | Deleted id=3, tuple_ptr={hex(deleted.ptr)}, decoded={decode(data=box_tuple.to_buf(deleted))}", None)

    # Iterate over all remaining tuples in the space
    say.say_info("PY | Iterating over space content:", None)
    with index_api.IndexIterator.new_iterator(index, IteratorType.ITER_ALL, encode([])) as iterator:
        while True:
            try:
                tup = iterator.next()
            except Exception as exc:
                if "No more items" in str(exc):
                    break
                raise
